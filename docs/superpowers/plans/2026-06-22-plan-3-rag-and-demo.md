...
│   ├── capture/
│   │   └── capture.ts            # MODIFY: extend ScreenshotCapture → captureStates(...) multi-state
│   ├── events/
│   │   └── notifier.ts           # in-memory Notifier: subscribe(cb)/emit(entry)
│   └── pipeline/
│       └── runJob.ts             # MODIFY: drive interactions, per-state vision, multi-screenshot Doc, REPLAY branch, wire hooks
├── prompts/
...
│   ├── DocView.tsx               # MODIFY: rehype-slug anchors; render MULTIPLE screenshots (Level 1)
│   └── ChangeEntryCard.tsx       # MODIFY: before/after = default vs activated screenshot (Level 1)
├── components/shell/
│   └── NotificationProvider.tsx  # MODIFY: subscribe to ${NEXT_PUBLIC_BOT_URL}/events
...
## Phase A — RAG Search + Live Notifications
...
## Phase B — Interactive Capture ("Level 1") — fixes the dry-run gap
...
- [ ] **Step 1:** Write `demo-runbook.md` reflecting the **proven** setup:
  ...
  - **Narration (the real 8-stage `runJob` log):** PR detected → context → analyze → capture (now **multi-state**: default + click→activated) → vision ✓ (per state) → write → publish → Done → portal shows **v4 doc with both screenshots** + the **🔴 critical** What's New entry + the **live toast** → **RAG search** "How do I trigger the shark siren?" → cited answer → jump to section.
  ...
  - **Reset to re-demo:** revert the PR merge on `main` (forward commit); the bot's v4 commit is local-only (`git reset --hard origin/demo/shark-siren`); restart servers.
...
**Spec coverage:** RAG (§8) → Tasks 1–4, 6 (+ optional vector Task 8); §14 vector-store question → resolved (keyword default, vector optional); grounded/always-current/graceful-degradation → grounded prompt + `onIndexRebuild` rebuild + "offline" state; live notification on `critical/high` (§6,§9) → Task 5; demo on a real PR with downgrades + safety (§10) → Tasks 7 & 10 (proven live); agent surface (§13.1) → optional Task 9. **Interactive Capture (Phase B)** fixes the dry-run gap and is the headline upgrade — extends the `ScreenshotCapture` seam; Computer Use stays the roadmap.
...
**Type consistency:** `RagAnswer`/`SearchResult`/`ChangeEntry`/`Doc`/`Severity` are `@surf/types`; `RagSynthesisSchema`/`Retriever`/`CapturedState` are engine-internal; `DiffAnalysisSchema.interactions` is additive (default `[]`, so non-interactive changes are unaffected). Anchors consistent (`github-slugger` ↔ `rehype-slug`).
